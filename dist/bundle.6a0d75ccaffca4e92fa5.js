...
function submitForm(form, listEvents, keys) {
  const newEvent = [...form].reduce((acc, curr) => {
    ...
    acc[curr.id] = curr.id === keys.participants ? [...curr.selectedOptions] : [curr.value];
    return acc;
  }, {});
...
async function CreateEvent(links) {
  ...
  selects.forEach(({
    ...
    if (fieldSet) {
      const input = (0,_select_select__WEBPACK_IMPORTED_MODULE_0__.default)(id, list, multiple);
      fieldSet.append(...input.children);
    }
  });
  createEvent.querySelector('button[type="submit"]').addEventListener('click', e => submitForm(e.target.form, listEvents, eventKeys));
  return createEvent;
}
...
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("6a0d75ccaffca4e92fa5")
/******/ 	})();
/******/ 	
